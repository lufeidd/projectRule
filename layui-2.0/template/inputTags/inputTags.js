...
layui.define(['jquery','layer'],function(exports){
  ...
  Class.prototype.init = function(){
    ...
    $.each(options.content,function(index,item){
      spans +='<span class="tagsSpan"><em class="tagsText">'+item+'</em><button type="button" class="close">×</button></span>';
    ...
    options.elem.keypress(function(event){
      ...
      if(keynum == '13'){
        ...
        if(options.content.indexOf($val) == -1){
          ...
          that.render()
          spans ='<span class="tagsSpan"><em class="tagsText">'+$val+'</em><button type="button" class="close">×</button></span>';
          options.elem.before(spans)
        }
